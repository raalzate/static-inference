#!/usr/bin/env bash
# CLI fallback runner for the static-inference analyzer.
# Use when the MCP server is not registered in this Claude Code session.
#
# JAR resolution order:
#   1. STATIC_INFERENCE_JAR env var
#   2. Parse .mcp.json (project root or any ancestor) for
...
find_jar_in_mcp_json() {
  local dir="$1"
  while [[ "${dir}" != "/" ]]; do
    if [[ -f "${dir}/.mcp.json" ]] && command -v jq >/dev/null; then
      local jar
      jar="$(jq -r '.mcpServers["static-inference"].args[]? | select(test("\\.jar$"))' "${dir}/.mcp.json" 2>/dev/null | head -1)"
      if [[ -n "${jar}" && -f "${jar}" ]]; then
        echo "${jar}"
        return 0
      fi
...
elif found_jar="$(find_jar_in_mcp_json "$(pwd)")"; then
  JAR="${found_jar}"
...
Options to resolve:
  ...
  - Add a .mcp.json at the project root with the server's JAR path
    (see references/mcp-tools.md or build-setup.md)
