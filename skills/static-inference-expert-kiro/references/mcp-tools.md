...
When the MCP server is registered in `.kiro/settings/mcp.json`, tools appear in the Kiro MCP Servers panel under the `static-inference` server. Kiro invokes them by tool name (no namespace prefix):
...
To auto-approve all tool calls without per-call confirmation, add `"autoApprove": ["*"]` in the server entry of `.kiro/settings/mcp.json`. For selective approval, list individual tool names.

If tools are not visible, the server is not registered or not running — check the MCP Servers tab in the Kiro panel, then fall back to CLI via `scripts/run-analysis.sh`.
