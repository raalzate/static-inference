---
name: static-inference-expert-kiro
description: >-
...
> **Distribution**: this skill ships *with the static-inference MCP*. Copy this folder into `.kiro/skills/static-inference-expert-kiro/` of the project to analyze. The skill locates the JAR via `.kiro/settings/mcp.json`, env vars, or explicit args. See [INSTALL.md](./INSTALL.md).
...
1. **MCP server registered** — `.kiro/settings/mcp.json` lists a `static-inference` server entry pointing to the analyzer JAR. If absent, guide the user through [references/build-setup.md](./references/build-setup.md) §"MCP registration".
2. **MCP tools visible** — in this Kiro session look for `analyze_project` under the `static-inference` MCP server in the Kiro panel. If missing, the server failed to start: surface the diagnostic in `references/build-setup.md` §"Verification" and STOP.
...
Fallback CLI path (no MCP session). Requires either `STATIC_INFERENCE_JAR` env var, a `.kiro/settings/mcp.json` with the analyzer's JAR path, or `STATIC_INFERENCE_HOME` pointing to the analyzer repo:

```bash
bash .kiro/skills/static-inference-expert-kiro/scripts/run-analysis.sh <projectPath> [outputDir]
```
...
3. Read `.csproj`, save original to `pending_restore`. Append before `</Project>`:
   ...
   ```
4. Re-run `analyze_project` with the same `projectPath`.
5. Evaluate:
